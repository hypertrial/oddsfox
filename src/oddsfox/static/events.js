...
async function detail(identity,focus=true){
 ...
 if(d.data.combination)box.append(el("p","Listed combination · legs may be explained; formal combination verification is unsupported.","condition"));
 if(d.explanation){box.append(el("h3","What the rules say"),el("p","AI explanation · UNREVIEWED","badge warning"));for(const fact of d.explanation.data.facts){const section=el("div",undefined,"fact");section.append(el("span",fact.topic,"badge"),el("p",fact.text));for(const source of fact.citations)section.append(citation(source));box.append(section);}for(const gap of d.explanation.data.gaps)box.append(el("p",gap,"condition"));}
 else box.append(el("p",d.qualification==="unknown"?"Analysis is not queued until lifetime volume can be established.":"Local explanation pending. Source evidence is available below.","empty"));
 box.append(el("h3","Suggested cross-venue matches"));if(d.suggestions){box.append(el("p",d.suggestions.data.coverage,"small muted"));if(d.suggestions.data.omitted_pending_explanations?.length)box.append(el("p",`${d.suggestions.data.omitted_pending_explanations.length} candidates await explanations; matches will expand progressively.`,"condition"));for(const match of d.suggestions.data.matches){const section=el("div",undefined,"card");section.append(el("span",`${match.relationship} · UNREVIEWED`,"badge warning"),el("p",match.reason));for(const source of match.citations)section.append(citation(source));box.append(section);}if(!d.suggestions.data.matches.length)box.append(el("p","No assessed matches in the current candidate set.","muted"));}else box.append(el("p","Candidate analysis pending. A suggested match never establishes equivalence.","muted"));
 ...
 box.append(el("h3","Captured markets & governing evidence"));for(const c of d.contracts){const section=keyedDetails(c.data.metadata.title||c.logical,"contract:"+c.id);for(const [key,id]of Object.entries(c.data.text_artifacts)){const a=el("a",key);a.href=`/api/artifacts/${id}`;a.dataset.focusKey=`contract:${c.id}:${key}`;const p=el("p");p.append(a);section.append(p);}section.append(disclosure("Governing documents",c.data.references,c.id+":documents"));const review=el("a","Open this contract in the review workspace");review.href=`/research?contract=${encodeURIComponent(c.id)}`;review.dataset.focusKey=`review:${c.id}`;section.append(review);box.append(section);}if(!d.contracts.length){const review=el("a","Open the review workspace");review.href="/research";review.dataset.focusKey="review";box.append(review);}box.append(disclosure("Volume provenance and discovery evidence",d.data));box.dataset.identity=identity;box.dataset.snapshot=snapshot;
...
$("pause").addEventListener("click",async()=>{try{await api("/api/sync/pause",{paused:!paused});await refresh();}catch(e){notice(e.message,true);}});
