...
pub fn run_quality_checks(out: &std::path::Path) -> Result<Vec<DataQualityRecord>> {
    ...
    let missing_tokens_sql = format!(
        "SELECT market_id FROM read_parquet('{outcomes_glob}') WHERE token_id IS NULL"
    );
    ...
    let unresolved_sql = format!(
        "SELECT market_id FROM read_parquet('{markets_glob}') WHERE closed = true AND resolved = false"
    );
