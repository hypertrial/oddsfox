...
SELECT market_id, question, volume_24h, liquidity, source
FROM bronze_markets
ORDER BY volume_24h DESC NULLS LAST
LIMIT 20;

-- 2. Active markets only
SELECT market_id, question, volume_24h, close_time
FROM bronze_markets
WHERE active = true
ORDER BY volume_24h DESC NULLS LAST
LIMIT 20;

-- 3. Latest price per outcome for a market (replace market_id)
...
-- 4. Price history for a token (replace token_id)
SELECT ts, price, fidelity_minutes
FROM bronze_prices
WHERE token_id = 'YOUR_TOKEN_ID'
ORDER BY ts;

-- 5. Markets, outcomes, and recent prices
...
-- 6. Market count by source
SELECT source, COUNT(*) AS markets
FROM bronze_markets
GROUP BY source
ORDER BY markets DESC;

-- 7. Kalshi markets only
...
-- 9. Resolved markets since a date
